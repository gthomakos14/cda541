...
```{r}
train_err_store <- matrix(rep(NA, 9))
test_err_store <- matrix(rep(NA, 9))
y_true_train = df_train$rating
y_true_test = df_test$rating
for (i in 1:9){
  # Using only exhaustive subset selection.
    y_hat_train = predict(regit_exh, newdata = df_train, id = i)
    y_hat_test = predict(regit_exh, newdata = df_test, id = i)
...
print('Train Error at n variables:')
train_err_store
print('Test Error at n variables:')
test_err_store
```
